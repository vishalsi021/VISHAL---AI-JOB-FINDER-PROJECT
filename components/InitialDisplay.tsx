...
const GrowthBadge: React.FC<{ growth: 'Hot' | 'Growing' | 'Stable' }> = ({ growth }) => {
    const styles = {
        Hot: { icon: 'fa-fire', color: 'text-red-400', bg: 'bg-red-900/50' },
        Growing: { icon: 'fa-chart-line', color: 'text-green-400', bg: 'bg-green-900/50' },
        Stable: { icon: 'fa-check-circle', color: 'text-blue-400', bg: 'bg-blue-900/50' }
    };
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
                <div className="lg:col-span-3 bg-gray-800/50 p-6 rounded-xl border border-gray-700">
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="font-semibold text-lg flex items-center text-teal-300">
                         ...
                         <div className="flex items-center gap-3">
                            ...
                            <button
                                onClick={onRefresh}
                                disabled={isLoading}
                                className="text-gray-400 hover:text-teal-300 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {jobs.map(job => (
                           <button
                             ...
                             className="text-left p-4 bg-gray-800 rounded-lg hover:bg-teal-500/10 hover:border-teal-600 border border-gray-700 transition-all duration-300 group space-y-3"
                           >
                            <div className="flex justify-between items-start">
                                <div>
                                    <p className="font-bold text-gray-100 group-hover:text-teal-300">{job.title}</p>
                                    <p className="text-sm text-yellow-300/80 font-semibold">{job.salaryRange}</p>
                                </div>
                                <GrowthBadge growth={job.growth} />
                            </div>
                            <div>
                                <p className="text-xs font-semibold text-gray-400 mb-1.5">Top Industries:</p>
                                <div className="flex flex-wrap gap-x-3 gap-y-1">
                                    {job.topIndustries.map(industry => (
                                        <div key={industry} className="flex items-center text-xs text-gray-300">
                                            <i className={`fas ${getIndustryIcon(industry)} mr-1.5 w-4 text-center text-gray-400`}></i>{industry}
                                        </div>
                                    ))}
                                </div>
                            </div>
                             <div>
                                <p className="text-xs font-semibold text-gray-400 mb-1.5">Key Skills:</p>
                                <div className="flex flex-wrap gap-1">
                                    {job.keySkills.map(skill => (
                                        <span key={skill} className="bg-gray-700 text-gray-200 px-2 py-0.5 rounded-md text-xs font-medium">{skill}</span>
                                    ))}
                                </div>
                            </div>
                           </button> 
                 ...
                 <div className="lg:col-span-2 bg-gray-800/50 p-6 rounded-xl border border-gray-700">
                    <h3 className="font-semibold text-lg mb-4 flex items-center text-yellow-300">
                        <i className="fas fa-star mr-3"></i>
                        Top In-Demand Skills
                    </h3>
                    <div className="flex flex-wrap gap-3">
                        {skills.map(skill => (
                           <div key={skill} className="flex items-center bg-gray-700 rounded-lg text-sm font-medium overflow-hidden">
                               <span className="text-gray-200 pl-3 pr-2 py-1.5">{skill}</span>
                               <button
                                   onClick={() => onSkillTarget(skill)}
                                   className="text-yellow-400 hover:text-black bg-gray-600 hover:bg-yellow-400 font-semibold px-2.5 py-1.5 h-full transition-colors duration-200"
                                   title={`Target '${skill}'`}
                               >
                                   Target
                               </button>
