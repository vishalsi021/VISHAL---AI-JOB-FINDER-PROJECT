...
export const SkillCard: React.FC<SkillCardProps> = ({ skill }) => {
  return (
    <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 hover:border-teal-400 dark:hover:border-teal-500 transition-all duration-300 h-full flex flex-col">
      <div className="flex-grow">
        <div className="flex justify-between items-start">
            <h4 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{skill.name}</h4>
            <span className="text-sm font-bold text-teal-400 bg-teal-50 dark:bg-teal-900/50 px-2 py-1 rounded-md">{skill.relevance}/10</span>
        </div>
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">{skill.description}</p>
      </div>
      <div className="mt-4">
        <p className="text-xs text-gray-500 dark:text-gray-500 mb-1">Relevance</p>
